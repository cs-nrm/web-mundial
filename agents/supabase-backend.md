...
## Rol
Gestiona la base de datos Supabase: esquema, migraciones, queries y endpoints API del servidor.
...
- `POST /api/album/redimir`
- `PUT /api/perfil/guardar`
- `GET|POST /api/perfil/playlists`
- `GET|PUT|DELETE /api/perfil/playlists/[id]`
- `POST /api/perfil/playlists/[id]/canciones`

## Lo que NO hace este agente
- No modifica flujos de autenticación (→ `auth-sessions`)
- No modifica UI de páginas (→ `page-builder` o `component-creator`)
- No toca la configuración de Astro o deploy (→ `deploy-ops`)
